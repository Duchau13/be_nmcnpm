  ...
  try {
    ...
    if (order.status == 0) {
      ...
      return res.status(400).json({
        status: "fail",
        message: "Không thể cập nhật trạng thái. Đơn hàng không hợp lệ!",
      });
    }
  } catch (error) {
    console.error(error);
...
const confirmOrder2 = async (req, res) => {
  ...
  try {
    ...
    if (!order) {
      return res.status(404).render("order/order-notification", {
        message: "Đơn hàng không tồn tại!",
      });
    }

    if (order.status === 0) {
      order.status = 1; // Update status to Đã xác nhận
      await order.save();
      return res.redirect('/orders'); // Redirect to /orders
    } else {
      return res.status(400).render("order/order-notification", {
        message: "Thao tác thất bại. Đơn hàng đã được xác nhận hoặc có trạng thái khác!",
      });
    }
  } catch (error) {
    console.error('Error confirming order:', error);
    return res.status(500).render("order/order-notification", {
      message: "Thao tác thất bại do lỗi server!",
    });
  }
};
