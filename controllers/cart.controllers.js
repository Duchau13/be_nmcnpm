...
const checkout = async (req, res) => {
  ...
  try {
    ...
    if (date.getHours() <= 7 || date.getHours() >= 22) {
      return res.status(400).json({ message: "Ngoài thời gian phục vụ. Vui lòng đặt hàng lại sau 7h00 và trước 22h00!" });
    }
    ...
    if (itemInCartList.length) {
      ...
      let random = getDistanceFromLatLonInKm(userLat, userLng, storeLat, storeLng);
      if (random < 2) {
        random = shipping_partner.unit_price * 5;
      } else if (random >= 2 && random < 5) {
        random = shipping_partner.unit_price * 5 + 5000;
...
const confirmOrder = async (req, res) => {
  ...
  try {
    ...
    if (order.status == 1) {
      order.status = 2;
      await order.save();
      return res.status(200).render("order/order-notification", {
